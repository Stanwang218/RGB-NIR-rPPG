...
# compute heart rate for each clip
def compute_metric_per_clip(clip, diff_flag=True ,method='FFT'):
    low_pass = 0.75
    high_pass = 2.5
    order = 2
    fs = 30
    lambda_value = 100
    if diff_flag:
        clip = _detrend(np.cumsum(clip), lambda_value)
    else:
        clip = _detrend(clip, lambda_value)
    # butter worth filter
    [b, a] = butter(order, [low_pass / fs * 2, high_pass / fs * 2], btype='bandpass')
    
    clip = scipy.signal.filtfilt(b, a, np.double(clip))
    clip = _standardize_signal(clip)
    
    hr_label = _calculate_hr(clip, fs=fs, low_pass=low_pass, high_pass=high_pass, method=method)
    return hr_label


def one_process_task(path, base_name):
    hr_list = []
    base_name_list = []
    label_dir = os.path.join(path, base_name)
    label = np.load(label_dir)
    temp_hr = []
    wind_sz = 224
    step = wind_sz
    for i in range(0, len(label) - wind_sz, step):
        label_window = label[i: i + wind_sz]
        temp_hr.append(compute_metric_per_clip(label_window))
    hr_list.append(temp_hr)
    print(base_name)
    return base_name, temp_hr

def multiprocess_hr_extract(path, wind_sz = 224, step = 224):
    label_data = glob.glob(f"{path}/*label*.npy")
    hr_list = []
    p = Pool(64)
    base_name_list = []
    csv_path = '/data/PreprocessedData/raw/UBFC_hr_serve.csv'
    
    # print([path] * len(label_data))
    for base_name, temp_hr in p.starmap(one_process_task, zip([path] * len(label_data), label_data)):
        hr_list.append(temp_hr)
...
def batch_hr_extract(path, wind_sz = 224, step = 224):
    label_data = glob.glob(f"{path}/*label*.npy")
    hr_list = []
    ...
    df = pd.DataFrame(df)
    csv_path = '/data/PreprocessedData/UBFC_new/UBFC_hr_serve.csv'
    df.to_csv(csv_path)

if __name__ == '__main__':
    batch_hr_extract('/data/PreprocessedData/UBFC_RAW_RAW/')
    exit(0)