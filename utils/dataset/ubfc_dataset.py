...
class MSTmap_UBFC_cut(Dataset):
    ...
    def split_dataset(config):
        ...
        dataset_path = '/data/PreprocessedData/UBFC_new'
    ...
    def __init__(self, root='/data/PreprocessedData/UBFC_new',
    ...
    def __getitem__(self, index):
        ...
        for _map in self.maps:
            if _map == "CHROM":
                ...
                pos_path = os.path.join(self.pos_path, map_name)
                rgb_path = os.path.join(self.rgb_path, map_name)
                temp_rgb = cv.imread(rgb_path)
                temp_chrom = cv.imread(chrom_path)
                temp_pos = cv.imread(pos_path)
                # print(temp_chrom[:, :, 2, np.newaxis].shape)
                # print(temp_pos[:, :, 2, np.newaxis].shape)
                # print(temp_rgb[:, :, 1, np.newaxis].shape)
                map_list.extend([temp_chrom[:, :, 2, np.newaxis], temp_pos[:, :, 2, np.newaxis], temp_rgb[:, :, 1, np.newaxis]])
