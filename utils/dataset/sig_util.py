...
def compute_metric_per_clip(clip):
    low_pass = 0.75
    high_pass = 2.5
    order = 2
    fs = 30
    ...
    hr_label = _calculate_hr(clip, fs=fs, low_pass=low_pass, high_pass=high_pass)
    return int(hr_label + 0.5)

def hr_extract(label_path, wind_sz = 224, step = 10, use_bandpass = True):
    hr_list = []
    low_pass = 0.75
    high_pass = 2.5
    order = 2
    ...
    label_list = [label[i: i + wind_sz] for i in range(0, len(label) - wind_sz, step)]
    hr_list = [compute_metric_per_clip(i) for i in label_list]
    return hr_list
