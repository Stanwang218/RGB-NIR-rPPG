...
class HR_estimator_multi_task_STmap(nn.Module):
    def __init__(self, video_length = 300, num_channel=6):
    ...
    def forward(self, x):
        ...
        hr, feat_out, feat = self.extractor(x) # feat_out: output of encoder(latent variable)
...
class HR_disentangle(nn.Module):
    def __init__(self, video_length = 300, num_channel = 6,decov_num = 1):
        super(HR_disentangle, self).__init__()

        self.extractor = HR_estimator_multi_task_STmap(video_length, num_channel)
        self.Noise_encoder = resnet18_part()
        self.Noise_encoder.conv1 = nn.Conv2d(num_channel, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.decoder = Generator(conv_dim=128, repeat_num=decov_num, num_channel=num_channel)
    ...
    def forward(self, img):
        ...
        return feat_hr, feat_n, hr, img, ecg # feature_hr, feature_noise, hr, reconstructed map, rPPG

class HR_disentangle_cross(nn.Module):
    def __init__(self, video_length = 300, num_channel = 6):
        super(HR_disentangle_cross, self).__init__()

        self.encoder_decoder = HR_disentangle(video_length, num_channel, decov_num = 1)
        self.test = False
        self.video_length = video_length
        self.poolspa = nn.AdaptiveAvgPool2d((1, int(self.video_length)))
        self.ecg_conv = nn.Conv2d(32, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, img):
        ...
        # original feature, original noise, original predicted heart rate, reconstructed map (1 - 4)
        # fake group (5 - 7), heart rate fake
        # group one index: 8
...
if __name__ == '__main__':
    ...
    # net = HR_disentangle()
    net = HR_disentangle_cross(video_length=224)
    
    summary(net, input_size=[1, 6, 224, 224])