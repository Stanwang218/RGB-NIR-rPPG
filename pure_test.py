...
def get_parser():
    ...
    parser.add_argument("--config_file", required=False, default='/mae_rppg/eccv_model/train/PURE/fold1.yaml')
    parser.add_argument("--double", action='store_true',)
    parser.add_argument("--linear_probe", action='store_true',)
    parser.add_argument("--dataset", default='PURE')
    return parser
...
train_dataset, test_dataset, valid_dataset = MSTmap_PURE_cut.split_dataset(dataset_config)
# print(train_dataset[0][0].shape)

if runner_config['task'] == 'test':
    if args.dataset == 'PURE':
        test_dataset = MSTmap_PURE_cut(map_list = dataset_config['map_type'])
    else:
        test_dataset = MSTmap_UBFC_cut(map_list = dataset_config['map_type'])
...
if args.double:
    ...
    net = HR_disentangle_cross(video_length);
...
if args.linear_probe:
    for name, param in net.named_parameters():
        if 'multi_modal_encoder_decoder.extractor1.extractor.fc' in name:
            print(name, param.shape)
            continue
...
def finte_tune():
    ...
    train_loss = 0;
    fps = torch.tensor([30])
    for batch_idx, (data, bvp, bpm) in tqdm(enumerate(train_loader)):
        ...
        if args.double:
            output_list, ecg, hr_feat_list, hr_n_list, img_out1, img_out2, idx1, idx2 = net(data)
        ...
        # loss_img_rgb = lossfunc_img(img_out1, data1) * lambda_img # reconstruction loss
        ...
        loss = loss_hr#  + loss_ecg + loss_img_rgb + loss_SNR;
        ...
        optimizer.step();
        print(f"{loss.item()}")
        # print('Train epoch: {:.0f}, it: {:.0f}, \n loss: {:.4f}, loss_hr: {:.4f}, loss_img_rgb: {:.4f}, \n loss_cross: {:.4f}, loss_snr: {:.4f}, loss_ecg: {:.4f}'.format(epoch, batch_idx,
        #                                                                                loss, loss_hr, loss_img_rgb, loss_cross, loss_SNR, loss_ecg));
    # valid
    val_loss = 0
    with torch.no_grad():
        for batch_idx, (data, bvp, bpm) in tqdm(enumerate(valid_loader)):
            data = Variable(data);
            bvp = Variable(bvp);
            bpm = Variable(bpm.view(-1,1));
            fps = Variable(fps.view(-1,1));
            ...
            # loss = loss + loss_cross;
            # print('Val epoch: {:.0f}, it: {:.0f}, loss: {:.4f}, loss_hr: {:.4f}, loss_img_rgb: {:.4f}, \n loss_cross: {:.4f}, loss_snr: {:.4f}, loss_ecg: {:.4f}'.format(epoch, batch_idx,
            #                                                                            loss, loss_hr, loss_img_rgb, loss_cross, loss_SNR, loss_ecg));
            print(loss.item())
...
def test():
    ...
    snr_all = []
    with torch.no_grad():
        for (data, bvp, hr) in test_loader:
